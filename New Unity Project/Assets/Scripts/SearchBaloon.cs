    ...
    public void Looting()
    {
        ...
        while (countLoot > 0)
        {
            ...
            if (!dataLoot.equipment)
            {
                ...
                panelLoot.GetChild(countLoot - 1).GetChild(0).gameObject.SetActive(true);
                panelLoot.GetChild(countLoot - 1).GetChild(1).gameObject.SetActive(true);
                panelLoot.GetChild(countLoot - 1).GetChild(2).gameObject.SetActive(true);

                panelLoot.GetChild(countLoot - 1).GetChild(0).GetComponent<Image>().sprite = dataLoot.img; // Изображение лута
                panelLoot.GetChild(countLoot - 1).GetChild(1).GetComponent<Text>().text = dataLoot.Name; // Название лута
                panelLoot.GetChild(countLoot - 1).GetChild(2).GetComponent<Text>().text = count.ToString(); // Количество лута
